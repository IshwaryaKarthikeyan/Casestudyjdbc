...
public class OrderProcessor implements IOrderManagementRepository {
    ...
    public int createOrderInDatabase(int userId) throws SQLException{
            stmt = conn.prepareStatement("insert into Orderr(userId) values(?)");
            stmt.setInt(1, userId);
            stmt.executeUpdate();
    ...
    public void addOrderItems(int orderId, List<Product> products) throws SQLException {
    	Order o=new Order();
        stmt = conn.prepareStatement("insert into Orderr values (?, ?)");
	...
	public void cancelOrder(int userId, int orderId) throws OrderNotFoundException, UserNotFoundException {
		  try {
	...
	public List<Product> getOrderByUser(User user) {
	    ...
	    try {
	        stmt = conn.prepareStatement("select * from Orderr where orderId in (select orderId from Orderr where userId = ?)");
	        stmt.setInt(1, user.getUserId());
	        ResultSet rs = stmt.executeQuery();
	        while (rs.next()) {
	            int productId = rs.getInt("productId");
	            // Fetch product details from Product table based on productId
